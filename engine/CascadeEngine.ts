 ...
 */
export function performCascade(grid: Grid, clusters: Cluster[]): { newGrid: Grid; step: CascadeStep } {
  ...
  for (let col = 0; col < cols; col++) {
    ...
    for (let row = writeRow; row >= 0; row--) {
      const newCell: GridCell = {
        symbolId: generateWeightedSymbol(),
        row,
        col,
        key: nextCellKey(),
      };
...
export function runFullCascade(
  ...
  while (currentClusters.length > 0) {
    ...
    // Perform cascade
    const { newGrid, step } = performCascade(currentGrid, currentClusters);
